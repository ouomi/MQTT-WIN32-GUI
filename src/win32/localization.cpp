...
std::wstring_view Text(AppLanguage language, UiText text) {
    if (language == AppLanguage::Chinese) {
        ...
        case UiText::MqttEventsDropped: return L"[系统] 事件队列超限，丢弃普通事件数：";
    ...
    case UiText::MqttEventsDropped: return L"[system] Event queue limit exceeded; ordinary events dropped: ";
